...
class Registry:
    ...
    def finish_apply(self, name: str, *, ok: bool, claims: dict[str, Claim], applied_commit: str|None, caller_arn: str|None, tofu_version: str|None, base_etag_after: str|None, summary: dict, run_id: str|None = None, released: Iterable[str] = (), targets: Iterable[str]|None = None) -> RegistryDoc   # targets (apply --only-claims) -> last_apply.only_claims = True + last_apply.targets (sorted); only_claims False otherwise
...
class OverlayService:
    ...
    def apply(self, *, auto_approve: bool, allow_stale: bool, allow_behind: bool, extra: list[str] = [], yes: bool = False, accept_drift: bool = False, only_claims: bool = False) -> Overlay   # console.confirm before acquire_claims unless auto_approve/yes; plan file unlinked afterwards; DriftError(4) when policy.drift is non-empty without accept_drift/only_claims; PolicyError when accept_drift in CI without yes, or only_claims with accept_drift; only_claims with drift/ignored -> second plan runner.plan(targets=sorted(policy.claims)), evaluated with the same inputs, plan.gate_targeted_plan, then applied (DESIGN §7.9); apply_targets: list[str]|None attribute = the target set of the last call (None = full plan applied)
