...
9. **Pass-through arguments** `-target`, `-replace`, `-refresh-only`, `-destroy`, `-state`, `-lock=false`, `-out` are rejected for `plan`/`apply`; `TF_CLI_ARGS*` variables (tofu's other way to inject arguments) are dropped from the environment of every tofu run.
